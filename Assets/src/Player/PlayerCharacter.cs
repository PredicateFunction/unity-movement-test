...
public class PlayerCharacter : MonoBehaviour, ICharacterController
{
    ...
    [SerializeField] private float sprintResponse = 3f;
    ...
    [SerializeField] private float sprintJumpSpeed = 16f;
    ...
    public void UpdateInput(CharacterInput input)
    {
        ...
        _requestedAirCrouch = _requestedCrouch && !_state.Grounded;
    ...
    public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        if (motor.GroundingStatus.IsStableOnGround)
        {
            ...
            if (moving && _isSprinting && crouching && (wasStanding || wasAir))
            {   
            ...
            {
                var speed = _state.Stance is Stance.Stand ? (_isSprinting ? sprintSpeed : walkSpeed) : crouchSpeed;
                var response = _state.Stance is Stance.Stand ? (_isSprinting ? sprintResponse : walkResponse) : crouchResponse;
        ...
        if (_requestedJump)
        {
            ...
            if (isGrounded || canCoyoteJump)
            {
                ...
                var targetVerticalSpeed = Mathf.Max(currentVerticalSpeed, _isSprinting ? sprintJumpSpeed : jumpSpeed);
    ...
    public void AfterCharacterUpdate(float deltaTime)
    {
        if (!_requestedCrouch && _state.Stance is not Stance.Stand)
        {
            ...
            bool isObstructed = motor.CharacterOverlap(
                pos,
                rot,
